/**
 * X8R algorithm (Weighted X8 with Randomized chain order)
 *
...
static void getX8RAlgoString(const uint32_t* prevblock, char *output)
{
	char *sptr = output;
...
// X8R CPU Hash (Validation)
extern "C" void x8r_hash(void *output, const void *input)
{
...
extern "C" int scanhash_x8r(int thr_id, struct work* work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	if (s_ntime != ntime) {
		getX8RAlgoString(&endiandata[1], hashOrder);
		s_ntime = ntime;
		s_implemented = true;
...
// cleanup
extern "C" void free_x8r(int thr_id)
{
	if (!init[thr_id])
		return;
